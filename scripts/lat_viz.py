...
def plot_k_lat_motifs(k, trip_df, motif_dic_list, add_suptitle=True):
    """
...
def plot_single_motif(trip_df, motif_dic, add_suptitle):
    ...
    plt.plot(trip_df["timestamp"], trip_df["ay"], "xkcd:grey", alpha=0.5)
    for temp_point in member_pointers:
        temp_member_df = trip_df.iloc[temp_point]
        plt.plot(temp_member_df["timestamp"], temp_member_df["ay"], "xkcd:dark grey")
    sns.scatterplot(
        x="timestamp",
        y="ay",
        hue="event_type",
        data=trip_df.loc[trip_df["event_type"] == 2],
        legend=False,
        palette=sns.xkcd_palette(["tangerine"]),
    )
    plt.title("Position of motif's members and center on full trip")
    plt.ylabel("Lateral acceleration (Gs)")
    plt.xlabel("Time (seconds)")
    plt.ylim(trip_df["ay"].min() - 0.01, trip_df["ay"].max() + 0.01)
    # subplot 2
    ...
    plt.plot(center_df["timestamp"], center_df["ay"], "xkcd:azure")
    plt.xlabel("Time (seconds)")
    plt.ylabel("Lateral acceleration (Gs)")
